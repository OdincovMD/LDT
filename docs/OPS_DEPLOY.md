...
HOST=172.0.0.1 # если у вас есть сервер, укажите его ip
DEV=allow all|deny all; # для разработки или развёртывания на локальной машине используйте allow all; для установке на сервере deny all;
```
