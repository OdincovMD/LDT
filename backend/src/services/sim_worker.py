"""
Фоновый воркер для симуляции стриминга сигналов из CSV и периодического вызова ML.
Читает t,bpm,uc; timestamp считается от старта; по окну 300 с дергает инференс с шагом stride_s и горизонтом H.
Сохраняет точки и предсказания через AsyncOrm.
ENV: CSV_PATH (demo.csv / demo.user.csv), ML_URL (/predict).
"""
